# Аналитический конвейер на базе Apache Airflow и Docker

Данный репозиторий содержит платформу для сбора, оркестрации и аналитической загрузки данных о температуре в Калининграде, реализованную с использованием контейнеризированного микросервисного подхода. Система автоматически собирает текущую погоду и прогнозы, оценивает точность прогнозов и предоставляет аналитические данные для мониторинга качества метеорологических предсказаний.

## Структура проекта:
```
root/
├─ .env
├─ .gitignore
├─ docker-compose.yml
├─ README.md
├─ app/
│ ├─ Dockerfile
│ ├─ requirements.txt
│ └─ main.py
├─ airflow/
│ ├─ Dockerfile
│ ├─ requirements-airflow.txt
│ ├─ dags/
│ │ └─ collect_weather_dag.py
│ │ └─ weather_elt.py
│ ├─ logs/
│ └─ plugins/
```

## Архитектура и компоненты

Проект развернут через **Docker Compose** и представляет собой стек из шести взаимосвязанных сервисов:

| Сервис | Технология | Роль |
| :--- | :--- | :--- |
| `app` | FastAPI (Python 3.11) | Сбор данных. Получает текущую погоду и 48-часовой прогноз для Калининграда через OpenWeatherMap API. |
| `mongodb` | Mongo 6 | Landing Zone. Хранение неструктурированных данных. |
| `postgres_analytics` | PostgreSQL 14 | Data Warehouse. Аналитическое хранилище структурированных данных (таблица `analytics.tickets`). |
| `Airflow Stack` | Apache Airflow | Оркестрация (ELT). Управляет DAGs для перемещения данных и администрирования. |
| `postgres` | PostgreSQL 14 | База метаданных Airflow. |
| `redis` | Redis 7 | Брокер сообщений для Celery Executor Airflow. |

## DAGs

### 1. `collect_weather_data` - DAG сбора данных

**Назначение**: Ежечасный сбор актуальных данных о погоде через внешнее API.

**Расписание**: `0 * * * *` (запуск каждый час в 00 минут)

**Задачи**:

**Данные, сохраняемые в MongoDB**:
- **Текущая погода** (коллекция `current_weather`):
  - Температура, ощущаемая температура
  - Влажность, давление, скорость и направление ветра
  - Описание погоды (ясно, облачно, дождь и т.д.)
  - Время сбора (`collected_ts`)
- **Прогнозы** (коллекция `forecasts`):
  - Прогнозируемые значения
  - Вероятность осадков
  - Время прогноза (`forecast_dt`) и время сбора прогноза (`collection_dt`)

**Особенности**:
- Использует мокированные данные при недоступности OpenWeatherMap API
- Обеспечивает отказоустойчивость через retry-логику
- Логирует успешность сбора и количество записей

---

### 2. `weather_elt` - Аналитический конвейер (ELT)

**Назначение**: Обработка собранных данных, загрузка в аналитическое хранилище и расчет точности прогнозов.

**Триггер**: Запускается автоматически после успешного выполнения `collect_weather_data`

**Архитектура ELT**:

#### 🔄 **Extract (Извлечение)**
| Задача | Описание |
|--------|----------|
| `extract_current_weather` | Извлекает данные о текущей погоде из MongoDB за последние 3 часа |
| `extract_forecasts` | Извлекает прогнозы из MongoDB за последние 3 часа |

#### 📥 **Load (Загрузка)**
| Задача | Описание |
|--------|----------|
| `load_current_weather` | Загружает фактические данные о погоде в таблицу `analytics.current_weather` в PostgreSQL |
| `load_forecasts` | Загружает данные прогнозов в таблицу `analytics.forecasts` в PostgreSQL |

#### ⚙️ **Transform & Analyze (Преобразование и Анализ)**
| Задача | Описание |
|--------|----------|
| `calculate_accuracy_metrics` | Ключевая аналитическая задача. Сравнивает прогнозы с фактическими данными:<br>• Находит прогнозы, сделанные более 1 часа назад<br>• Ищет соответствующие фактические измерения (±1 час)<br>• Рассчитывает метрики точности |
| `generate_accuracy_report` | Формирует сводный отчет о качестве прогнозов за последние 24 часа |


## Инструкция по запуску

Для развертывания платформы требуется установленный Docker и Docker Compose.

### Запуск и инициализация

Выполните команду:

```bash
docker-compose up -d --build
```

> Примечание: Для гарантии чистой инициализации базы данных (PostgreSQL и MongoDB), особенно после изменений в `.env` или предыдущих ошибок, используйте флаг `-v` (удаление томов): `docker-compose down -v && docker-compose up -d --build`.

### Доступ к интерфейсам

| Название                      | Порт  | Адрес                                    | Учетные данные          |
| :---------------------------- | :---- | :--------------------------------------- | :---------------------- |
| Airflow Webserver             | 8080  | [ссылка](http://89.169.170.56:8080/home) | admin / admin           |
| FastAPI App                   | 8081  | [ссылка](http://89.169.170.56:8081/docs) | N/A                     |
| Postgres Analytics | 5433  | [ссылка](http://89.169.170.56:5433/docs) | readonly / readonlypass |
| Mongo | 27017 | [ссылка](http://89.169.170.56:27017)     | readonly / readonlypass |

---

### URL подключения

**MongoDB:**

```env
MONGO_INITDB_ROOT_USERNAME=readonly
MONGO_INITDB_ROOT_PASSWORD=readonlypass
MONGO_HOST=mongodb.app.orb.local
MONGO_PORT=27017
MONGO_INITDB_DATABASE=prod

MONGO_URL=mongodb://readonly:readonlypass@mongodb.app.orb.local:27017/prod?authSource=prod
```

**Postgres Analytics:**

```env
POSTGRES_USER=readonly
POSTGRES_PASSWORD=readonlypass
POSTGRES_HOST=postgres.app.orb.local
POSTGRES_PORT=5433
POSTGRES_DB=analytics

POSTGRES_URL=postgresql://readonly:readonlypass@postgres.app.orb.local:5433/analytics
```
