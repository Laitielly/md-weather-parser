...
def load_current_weather(**context):
    ...
    try:
        ...
        try:
            ...
            if "duplicate key" not in str(schema_error) and "already exists" not in str(schema_error):
                raise schema_error
...
def calculate_accuracy_metrics(**context):
    ...
    for forecast in forecasts:
        forecast_id, forecast_dt_timestamp, collection_dt, temp_forecast, \
            humidity_forecast, pressure_forecast, weather_forecast = forecast
        ...
        if actual_data:
            temp_actual, humidity_actual, pressure_actual, weather_actual, verification_dt = actual_data
            ...
            verified_count += 1
            log.info(f"Verified forecast for {forecast_dt}: "
...
def generate_accuracy_report(**context):
    ...
    if stats and stats[0] > 0:
        total_forecasts, avg_temp_error, std_temp_error, avg_humidity_error, \
            avg_pressure_error, weather_matches, total_checked, \
            earliest_forecast, latest_forecast = stats
